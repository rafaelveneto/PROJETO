...
const NIVEL_LABEL = {
  nunca:'🔴 Nunca estudei', comecei:'🟡 Comecei', terminei:'🟠 Terminei sem confiança', aparar:'🟢 Aparar arestas'
};
...
function buildTodayTasks(totalMins) {
  ...
  const selected=[]; let budget=totalMins, rounds=0;
  while (budget>0 && rounds<100) {
    rounds++; let added=false;
    for (const q of queues) {
      if (!q.tasks.length||budget<=0) continue;
      const t=q.tasks[0];
      if (t.duracaoMin<=budget+15){ q.tasks.shift(); selected.push(t); budget-=t.duracaoMin; added=true; }
    }
    if (!added) break;
  }
  return selected;
...
window.goTab = function(name) {
  ...
  const titles={hoje:'Foco do Dia',historico:'Histórico',questoes:'Análise de Desempenho',
    progresso:'Progresso & Previsão',planejamento:'Planejamento',templates:'Templates'};
...
window.irParaDisciplinas = function() {
  goTab('planejamento');
  setTimeout(()=>switchPlan('disciplinas',document.getElementById('ptab-disciplinas')),60);
};
window.irParaImport = function(panel) {
  goTab('planejamento');
  setTimeout(()=>{
    switchPlan('importar',document.getElementById('ptab-importar'));
    setTimeout(()=>{ const btns=document.querySelectorAll('.imp-tab'); const idx=['nlm','form','xlsx'].indexOf(panel); if (idx>=0&&btns[idx]) btns[idx].click(); },60);
  },60);
};
...
// ============================================================
// RENDERIZAÇÃO DE TAREFAS — HELPERS
// ============================================================
function parseLeiSeca(text) {
  ...
  return `<table class="lei-seca-table">
    ...
    <tbody>${rows.map(r=>{ const p=r.split('|').map(s=>s.trim());
      return `<tr><td class="ls-dispositivo">${p[0]||''}</td><td class="ls-artigos">${p[1]||''}</td><td class="ls-motivo">${p[2]||''}</td></tr>`;
    }).join('')}</tbody></table>`;
}
function parseBizus(text) {
  if (!text) return '';
  return text.split('\n').filter(l=>l.trim())
    .map(b=>`<div class="bizu-card">${b.replace(/^[•\-]\s*/,'').trim()}</div>`).join('');
}
function parseKeywords(text) {
  if (!text) return '';
  return text.replace(/^[•\-]\s*/,'').split(',').map(k=>k.trim()).filter(Boolean)
...
function renderHoje() {
  ...
  const totalMins=horasHoje*60, allP=pendingTarefas(), horasSem=S.config.horasSemana.reduce((a,b)=>a+b,0);
  ...
  hojeList.innerHTML=todayTasks.length
    ...
    : `<div style="color:var(--tx3);font-size:13px;padding:12px 0">Configure a agenda na aba Planejamento → Trilha.</div>`;
}

// Acerto modal para tarefas de Questões
...
document.addEventListener('keydown',e=>{
  if (document.getElementById('modal-acerto')?.classList.contains('open')){
    if (e.key==='Enter') confirmarAcerto(true);
    if (e.key==='Escape') confirmarAcerto(false);
  }
});
...
function renderHistorico() {
  ...
  if (!done.length){ container.innerHTML=`<div class="empty-state"><div class="empty-state-icon">✅</div><div class="empty-state-title">Nenhuma tarefa concluída ainda</div><div class="empty-state-sub">Conclua tarefas na aba Hoje para vê-las aqui.</div></div>`; return; }
  const grouped={};
  done.forEach(t=>{ if (!grouped[t.discId]) grouped[t.discId]={nome:t.discNome,cor:t.discCor,tasks:[]}; grouped[t.discId].tasks.push(t); });
  let html=`<div style="font-size:12px;color:var(--tx3);margin-bottom:14px">${done.length} tarefa(s) concluída(s) no total</div>`;
  Object.values(grouped).forEach(g=>{
    html+=`<div class="card"><div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
      <div style="width:10px;height:10px;border-radius:50%;background:${g.cor}"></div>
      <span style="font-size:14px;font-weight:600;color:var(--tx)">${g.nome}</span>
      <span class="disc-badge">${g.tasks.length} concluída(s)</span></div>
      ${g.tasks.map(t=>`<div class="hist-task-row">
        <div class="hist-task-info">
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px">
            <span class="aula-badge">${t.aulaCod}</span>
            <span class="hist-task-title">${t.topico}</span>
          </div>
          <div class="hist-task-meta">${TYPES[t.type]?.label||t.type} · ${t.duracaoMin}min
            ${t.acertoQuestoes!=null?` · <span style="color:${t.acertoQuestoes>=70?'var(--gr)':'var(--re)'}">⚡ ${t.acertoQuestoes}% acerto</span>`:''}
          </div>
        </div>
...
window.renderQuestoes = function() {
  ...
  if (!S.questoes_history?.length){ container.innerHTML=`<div class="empty-state"><div class="empty-state-icon">📊</div><div class="empty-state-title">Nenhum dado importado ainda</div><div class="empty-state-sub">Importe suas estatísticas do TecConcursos para ver seu diagnóstico.</div><button class="btn btn-p" onclick="irParaImport('xlsx')">📂 Importar dados</button></div>`; return; }
  const now=new Date();
  ...
  if (!hist.length){ container.innerHTML=`<div class="empty-state"><div class="empty-state-sub">Sem dados no período selecionado.</div></div>`; return; }
  ...
  let acertoDis,totalDis,diffStr,diffCls;
  if (discNomeFilt){
    const ld=latest.disciplinas?.find(d=>d.nome.toLowerCase()===discNomeFilt.toLowerCase());
    const cfg=S.disciplinas.find(d=>d.nome.toLowerCase()===discNomeFilt.toLowerCase());
    acertoDis=ld?ld.pctAcerto+'%':'—'; totalDis=ld?ld.qResolvidas:'—';
    const diff=ld?(ld.pctAcerto-(cfg?.metaAcerto||80)).toFixed(1):0;
    diffCls=diff>=0?'vs-up':'vs-down'; diffStr=`${diff>=0?'▲':'▼'} ${Math.abs(diff)}% vs meta`;
  } else {
    const diff=(latest.pctGeral-MOCK_GLOBAL.avgAcerto).toFixed(1);
    acertoDis=latest.pctGeral+'%'; totalDis=latest.total;
    diffCls=diff>=0?'vs-up':'vs-down'; diffStr=`${diff>=0?'▲':'▼'} ${Math.abs(diff)}% vs média concorrência`;
  }
  let html=`<div class="bench-grid">
    <div class="bench-card"><div class="bench-title">${discNomeFilt||'Acerto Global'}</div>
      <div class="bench-val">${acertoDis}</div><div class="bench-vs ${diffCls}">${diffStr}</div></div>
    <div class="bench-card"><div class="bench-title">Nível</div>
      <div style="margin-top:8px">${getBelt(parseFloat(acertoDis)||0)}</div>
      <div class="bench-vs vs-flat" style="margin-top:12px;color:var(--tx3)">Amostra: ${totalDis} questões</div></div>
  </div>
  <div class="chart-container" style="height:260px;margin-bottom:20px"><canvas id="qChartAdvanced"></canvas></div>`;
  let criticas=[];
  (discNomeFilt?latest.disciplinas?.filter(d=>d.nome.toLowerCase()===discNomeFilt.toLowerCase()):latest.disciplinas||[]).forEach(d=>{
    const cfg=S.disciplinas.find(x=>x.nome.toLowerCase()===d.nome.toLowerCase());
    const meta=cfg?.metaAcerto||80;
    (d.topicos||[]).forEach(t=>{ if (t.qResolvidas>=10&&t.pctAcerto<meta){ const temR=cfg?getPendingForDisc(cfg.id).some(p=>['QUESTOES','REVISAO'].includes(p.type)):false; criticas.push({disc:d.nome,topico:t.nome,acerto:t.pctAcerto,meta,temR}); } });
  });
  if (criticas.length){ criticas.sort((a,b)=>a.acerto-b.acerto);
    html+=`<div class="card"><div class="ct" style="color:var(--re)">⚠️ UTI / Revisão Crítica</div>
      <div style="font-size:11px;color:var(--tx3);margin-bottom:14px">Tópicos com 10+ questões abaixo da sua meta.</div>
      <div class="rev-list">${criticas.slice(0,10).map(r=>`<div class="rev-item"><div>
        <div class="rev-topic">${r.topico}${r.temR?'<span class="tag-reforco" style="margin-left:6px">⚠️ REFORÇO</span>':''}</div>
        <div class="rev-disc">${r.disc}</div></div>
        <div class="rev-metrics"><div class="rev-metric-box"><span class="rev-metric-lbl">Acerto</span><span class="rev-metric-val val-danger">${Math.round(r.acerto)}%</span></div>
        <div class="rev-metric-box"><span class="rev-metric-lbl">Meta</span><span class="rev-metric-val" style="color:var(--tx2)">${r.meta}%</span></div></div>
      </div>`).join('')}</div></div>`; }
  container.innerHTML=html;
  setTimeout(()=>{
    ...
    let cAcerto,cVol;
    if (discNomeFilt){ cAcerto=hist.map(h=>{const d=h.disciplinas?.find(d=>d.nome.toLowerCase()===discNomeFilt.toLowerCase());return d?d.pctAcerto:null;}); cVol=hist.map(h=>{const d=h.disciplinas?.find(d=>d.nome.toLowerCase()===discNomeFilt.toLowerCase());return d?d.qResolvidas:null;}); }
    else { cAcerto=hist.map(h=>h.pctGeral); cVol=hist.map(h=>h.total); }
    const cor=discFilt?.cor||'#f5a623';
    qChartInstance=new Chart(ctx,{type:'line',data:{labels:hist.map(h=>h.label||new Date(h.importadoEm).toLocaleDateString('pt-BR')),datasets:[
      {label:'% Acerto',data:cAcerto,borderColor:cor,backgroundColor:cor+'1a',borderWidth:3,fill:true,tension:0.3,yAxisID:'y',spanGaps:true},
      {label:'Volume',type:'bar',data:cVol,backgroundColor:'rgba(59,130,246,0.15)',borderRadius:4,yAxisID:'y1'}
    ]},options:{responsive:true,maintainAspectRatio:false,scales:{y:{type:'linear',position:'left',min:0,max:100,grid:{color:'#27272a'}},y1:{type:'linear',position:'right',beginAtZero:true,grid:{drawOnChartArea:false}}}}});
  },100);
};
...
function renderProgresso() {
  ...
  const discProg=S.disciplinas.map(d=>{
    const dAll=allTarefas().filter(t=>t.discId===d.id), dDone=dAll.filter(t=>t.status==='concluida');
    return {disc:d,total:dAll.length,done:dDone.length,pct:dAll.length?Math.round(dDone.length/dAll.length*100):0};
  });
  let html=`<div class="sh"><div class="stit">Progresso & Previsão</div></div>
  ...
  <div class="card">
    ...
    <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;margin-bottom:18px" id="etaDisplay"></div>
    <div class="ct">⚡ Simulador — E se eu estudar...</div>
    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
      <input type="number" id="simHoras" value="${semAtual}" min="1" max="120"
        style="width:80px;text-align:center;font-size:20px;font-weight:700;font-family:monospace;padding:8px"
        oninput="simularProgresso()">
  ...
  <div class="card">
    ...
    ${discProg.map(d=>`<div class="disc-prog-row">
      ...
      <div class="disc-aula-dots">${sortedAulas(d.disc.aulas).map(a=>{
        const aD=a.tarefas.filter(t=>t.status==='concluida').length, aT=a.tarefas.length;
        const ap=aT>0?aD/aT:0;
        const col=ap===1?'var(--gr)':ap>0?'var(--acc)':'var(--bd2)';
        return `<div class="aula-dot" style="background:${col}" title="${a.codigo}: ${aD}/${aT}"></div>`;
      }).join('')}</div>
    </div>`).join('')}
  </div>`;
  container.innerHTML=html;
  simularProgresso();
  // ETA com horas atuais
  const etaEl=document.getElementById('etaDisplay'); if (!etaEl) return;
  const semanas=semAtual>0?Math.ceil(pendMins/(semAtual*60)):null;
  const eta=semanas?new Date(Date.now()+semanas*7*864e5):null;
  etaEl.innerHTML=semanas?`<div><div style="font-size:22px;font-weight:700;color:var(--acc);font-family:monospace">${eta.toLocaleDateString('pt-BR',{day:'2-digit',month:'long',year:'numeric'})}</div>
    <div style="font-size:11px;color:var(--tx3);margin-top:3px">${semanas} semanas com ${semAtual}h/semana atuais</div></div>`
    :`<div style="color:var(--tx3);font-size:13px">Configure a agenda na Trilha para ver a previsão.</div>`;
}
window.simularProgresso = function() {
  ...
  const semAtual=Math.round(S.config.horasSemana.reduce((a,b)=>a+b,0));
  const diff=horas-semAtual;
  const diffCor=diff>0?'var(--gr)':diff<0?'var(--re)':'var(--tx3)';
  const diffStr=diff>0?`+${diff}h a mais`:diff<0?`${diff}h a menos`:'mesmo ritmo atual';
  const res=document.getElementById('simResult'); if (!res) return;
  res.innerHTML=semanas?`<div class="sim-box">
    <div class="sim-result-val">${etaStr}</div>
    <div style="font-size:11px;color:var(--tx3)">${semanas} semanas · <span style="color:${diffCor}">${diffStr}</span> vs ritmo atual</div>
  </div>`:`<div style="font-size:13px;color:var(--tx3);margin-top:8px">Insira as horas para simular.</div>`;
};

// ============================================================
// PLANEJAMENTO — TRILHA ADAPTATIVA
// ============================================================
function calcMeta(totalMins) {
  return S.disciplinas.map(d=>{
    const stats=getLatestStatsForDisc(d.nome); let w=d.peso;
    if (stats){ if (stats.pctAcerto>=(d.metaAcerto||80)+5) w*=0.85; else if (stats.pctAcerto<(d.metaAcerto||80)-5) w*=1.25; }
    w*=(NIVEL_WEIGHT[d.nivelConhecimento||'nunca']||1.0);
    return {disc:d,rawW:d.peso,adjW:w,tasks:getPendingForDisc(d.id)};
...
window.renderAgendaGrid = function() {
  const hs=S.config.horasSemana, dias=['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'];
  const total=hs.reduce((a,b)=>a+b,0);
  document.getElementById('semanaGridContainer').innerHTML=`<div class="day-grid">${dias.map((d,i)=>`<div class="day-cell">
    <div class="day-nm">${d}</div>
    <input type="number" class="day-hi" value="${hs[i]}" min="0" max="16" onchange="updateHoraDia(${i},this.value)">
  </div>`).join('')}</div>
  <div class="meta-label" style="text-align:right;margin-top:8px">Total Planejado: <strong id="totalSemanaGrid">${total}h</strong></div>`;
};
window.updateHoraDia = function(idx,val) {
  S.config.horasSemana[idx]=Math.min(16,Math.max(0,parseInt(val)||0)); saveState();
  document.getElementById('totalSemanaGrid').textContent=S.config.horasSemana.reduce((a,b)=>a+b,0)+'h';
};
window.renderMeta = function() {
  ...
  data.forEach(d=>{
    ...
    const stats=getLatestStatsForDisc(d.disc.nome), acc=stats?stats.pctAcerto:'--';
    const ind=d.adjW>d.rawW?'<span style="color:var(--re);font-size:10px">↑ CARGA EXTRA</span>':d.adjW<d.rawW?'<span style="color:var(--gr);font-size:10px">↓ MANUTENÇÃO</span>':'';
    const temR=d.selected.some(t=>isReforco(d.disc.id,t.type));
    html+=`<div style="display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--bd);padding:8px 0">
      <div><div style="font-size:13px;color:var(--tx)">${d.disc.nome} ${ind}${temR?' <span class="tag-reforco">⚠️ REFORÇO</span>':''}</div>
      <div style="font-size:10px;color:var(--tx3)">Acerto: ${acc}% · Meta: ${d.disc.metaAcerto||80}%${d.disc.nivelConhecimento?' · '+NIVEL_LABEL[d.disc.nivelConhecimento]:''}</div></div>
      <div style="text-align:right"><div style="color:var(--acc);font-family:monospace;font-size:14px;font-weight:600">${fmtMin(d.alloc)}</div>
      <div style="font-size:9px;color:var(--tx3)">${d.selected.length} tarefa(s)</div></div></div>`;
  });
  html+=`</div>`;
  const totPend=pendingTarefas().length, totAloc=data.reduce((s,d)=>s+d.selected.length,0);
  const semEst=totAloc>0?Math.ceil(totPend/totAloc):'—';
  html+=`<div class="card" style="margin-top:12px"><div class="ct">🚀 Velocidade de Cruzeiro</div>
    <div class="sg" style="grid-template-columns:repeat(3,1fr);margin-bottom:0">
      <div class="sc"><span class="sv">${totPend}</span><span class="sl">Pendentes</span></div>
      <div class="sc"><span class="sv" style="color:var(--acc)">${totAloc}</span><span class="sl">Esta semana</span></div>
      <div class="sc"><span class="sv" style="color:${typeof semEst==='number'&&semEst<=4?'var(--gr)':'var(--yl)'}">${semEst}</span><span class="sl">Semanas p/ fechar</span></div>
    </div></div>`;
...
window.imprimirAgenda = function() {
  ...
  const data=calcMeta(totalMins); let allT=[]; data.forEach(d=>{ if (d.selected) allT.push(...d.selected); });
  allT=allT.sort(()=>Math.random()-0.5);
  ...
  let idx=0; hs.forEach((h,i)=>{
    if (!h){ html+=`<div class="print-day"><div class="print-day-header"><h2>${daysFull[i]}</h2><span>DESCANSO</span></div></div>`; return; }
    let bud=h*60, day=[];
    while (idx<allT.length&&bud>0){ const t=allT[idx]; if (t.duracaoMin<=bud+15){day.push(t);bud-=t.duracaoMin;idx++;}else{ if (!day.length){day.push(t);idx++;} break; } }
    html+=`<div class="print-day"><div class="print-day-header"><h2>${daysFull[i]}</h2><span>Meta: ${h}h</span></div>
      <ul class="print-task-list">${!day.length?'<li style="font-size:12px;color:#666">Nenhuma tarefa alocada.</li>':day.map(t=>`<li class="print-task-item">
        <div class="print-checkbox"></div><div class="print-task-content">
          <span class="print-disc-badge" style="background:${t.discCor}">${t.discNome}</span>
          <div class="print-task-title">[${t.aulaCod}] ${t.topico}</div>
...
window.parsearNLM = function() {
  ...
  blocks.forEach((block,i)=>{
    ...
    const leiSeca=lsM?lsM[1].replace(/Dispositivo\s*\|[^\n]+\n?/i,'').trim():'';
    const questoes=qfM?qfM[1].trim():'';
    const bizus=bzM?bzM[1].trim():'';
    const keywords=kwM?kwM[1].replace(/^[•\-]\s*/gm,'').trim():'';
    _parsedTasks.push({id:uid(),label:`Tarefa ${i+1}`,type:finalType,paginas,topico,subtopicos,duracaoMin:durMin,status:'pendente',statusEdital,comando,leiSeca,questoes,bizus,keywords});
  });
...
function buildNLMPreviewHTML(tasks) {
  let html=`<div class="parse-preview"><div style="display:flex;align-items:center;gap:8px;margin-bottom:14px"><span style="color:var(--gr);font-weight:600;font-size:13px">✔ ${tasks.length} tarefa(s) encontrada(s)</span></div>`;
  tasks.forEach((t,i)=>{
    const ti=TYPES[t.type]||TYPES.TEORIA, hasD=t.comando||t.leiSeca||t.questoes||t.bizus||t.keywords;
    html+=`<div class="nlm-task-card"><div class="nlm-task-header" onclick="toggleNLMCard(${i})">
      <div class="nlm-task-header-left"><span class="tag" style="color:${ti.cor};border-color:${ti.cor}30;background:${ti.cor}18;white-space:nowrap">${ti.label}</span>
      <span class="nlm-task-title-text">${t.topico}</span></div>
      <div class="nlm-task-header-right">
        ${t.statusEdital&&!t.statusEdital.toLowerCase().includes('não')?'<span class="edital-badge">✅ Edital</span>':''}
        <span class="nlm-task-detail-meta">📄 ${t.paginas}</span>
        <span class="nlm-task-detail-meta">⏱ ${t.duracaoMin}m</span>
        ${hasD?'<span class="nlm-expand-btn">▾ ver</span>':''}
      </div></div>
      ${t.subtopicos?`<div style="padding:0 12px 8px;font-size:11px;color:var(--tx3)">${t.subtopicos}</div>`:''}
      ${hasD?`<div class="nlm-task-details" id="nlm-card-${i}">
        ${t.comando?`<div class="nlm-detail-row"><span class="nlm-detail-label">📖 Comando</span><p>${t.comando}</p></div>`:''}
        ${t.leiSeca?`<div class="nlm-detail-row"><span class="nlm-detail-label">⚖️ Lei Seca</span><pre>${t.leiSeca}</pre></div>`:''}
        ${t.questoes?`<div class="nlm-detail-row"><span class="nlm-detail-label">📝 Questões</span><p style="white-space:pre-line">${t.questoes}</p></div>`:''}
...
window.renderForms = function() {
  ...
  document.getElementById('tfList').innerHTML=tfForms.map((t,i)=>`<div class="tblock" id="tf-${t.id}">
    <div class="tbh"><span style="color:var(--acc);font-weight:600;font-size:12px">Tarefa ${i+1}</span>
      ${tfForms.length>1?`<button class="btn btn-g" style="padding:4px 10px;font-size:11px" onclick="removeTF('${t.id}')">✕ Remover</button>`:''}
    </div>
    <div class="tf-fields-grid">
      ...
      <div class="fg full"><label>Tópico Principal</label><input id="tp-${t.id}" placeholder="Ex: Processo Legislativo"></div>
      <div class="fg full"><label>Comando de Estudo</label><textarea id="cm-${t.id}" rows="2" placeholder="Leia e esquematize..."></textarea></div>
      <div class="fg full"><label>Lei Seca — Dispositivo | Artigos | Motivo</label><textarea id="ls-${t.id}" rows="3" placeholder="CF/88 | Art. 51 | ..."></textarea></div>
      ...
      <div class="fg full"><label>Palavras-chave</label><input id="kw-${t.id}" placeholder="palavra1, palavra2..."></div>
    </div></div>`).join('');
};
window.salvarManual = function() {
  ...
  const tarefas=tfForms.map((t,i)=>({id:uid(),label:`Tarefa ${i+1}`,type:document.getElementById('ty-'+t.id).value,
    statusEdital:document.getElementById('ed-'+t.id).value,paginas:document.getElementById('pg-'+t.id).value,
    topico:document.getElementById('tp-'+t.id).value||`Tarefa ${i+1}`,duracaoMin:parseInt(document.getElementById('du-'+t.id).value)||60,
    comando:document.getElementById('cm-'+t.id).value,leiSeca:document.getElementById('ls-'+t.id).value,
    questoes:document.getElementById('qf-'+t.id).value,bizus:document.getElementById('bz-'+t.id).value,
    keywords:document.getElementById('kw-'+t.id).value,status:'pendente'}));
  d.aulas.push({id:uid(),codigo:cod,titulo:tit,tarefas}); saveState();
  showToast(`Aula salva em "${d.nome}"!`); irParaDisciplinas(); renderAll();
};
  ...
  reader.onload=e=>{
    try {
      ...
      for (let i=1;i<data.length;i++){
        ...
        const nome=String(row[idxN]).trim(),hier=String(row[idxH]||'').trim(),qR=Number(row[idxQ])||0,qA=Number(row[idxA])||0,pct=Number(row[idxP])||0;
        if (hier===''){ cur={nome,qResolvidas:qR,acertos:qA,pctAcerto:pct,topicos:[]}; discs.push(cur); }
        else if (cur){ cur.topicos.push({nome,qResolvidas:qR,acertos:qA,pctAcerto:pct}); totQ+=qR; totA+=qA; }
      }
    ...
    } catch(err){ showToast('Erro ao processar planilha. Exporte "Por Tópicos" no TecConcursos.','error'); }
  };
  reader.readAsArrayBuffer(file);
};
function renderXlsxPreview(data) {
  const weak=data.disciplinas.reduce((acc,d)=>{ const cfg=S.disciplinas.find(x=>x.nome.toLowerCase()===d.nome.toLowerCase()); const meta=cfg?.metaAcerto||80; return acc+(d.topicos||[]).filter(t=>t.pctAcerto<meta&&t.qResolvidas>=5).length; },0);
  const cor=data.pctGeral>=70?'var(--gr)':'var(--re)';
  document.getElementById('xls-preview').innerHTML=`<div style="color:var(--tx3);font-size:11px;margin-bottom:8px;font-weight:600;letter-spacing:.06em;text-transform:uppercase">Preview — ${data.label}</div>
  <div class="tec-preview-grid">
    <div class="tec-preview-card"><div class="tec-preview-val">${data.total}</div><div class="tec-preview-lbl">Total de Questões</div></div>
    <div class="tec-preview-card"><div class="tec-preview-val" style="color:${cor}">${data.pctGeral}%</div><div class="tec-preview-lbl">% de Acerto</div></div>
    <div class="tec-preview-card"><div class="tec-preview-val">${data.disciplinas.length}</div><div class="tec-preview-lbl">Matérias</div></div>
    <div class="tec-preview-card"><div class="tec-preview-val" style="color:var(--re)">${weak}</div><div class="tec-preview-lbl">Pontos Fracos</div></div>
  </div>
...
window.confirmarXlsx = function() {
  if (!_xlsxParsed) return; S.questoes_history.push(_xlsxParsed); saveState();
  const label=_xlsxParsed.label; _xlsxParsed=null;
  document.getElementById('xls-preview').innerHTML=''; document.getElementById('xls-file').value='';
  document.getElementById('xls-label').value=''; document.getElementById('file-upload-text').textContent='📂 Clique para selecionar o arquivo';
  showToast(`"${label}" importado!`); goTab('questoes');
};
window.cancelarXlsx = function() {
  _xlsxParsed=null; document.getElementById('xls-preview').innerHTML=''; document.getElementById('xls-file').value='';
...
function renderDiscList() {
  ...
  if (!S.disciplinas.length){ container.innerHTML=`<div class="empty-state"><div class="empty-state-icon">📚</div><div class="empty-state-title">Nenhuma disciplina cadastrada</div><div class="empty-state-sub">Clique em <strong>+ Nova Disciplina</strong> para começar.</div></div>`; return; }
  container.innerHTML=S.disciplinas.map(d=>`<div class="disc-card" id="disc-${d.id}">
    ...
    <div id="disc-edit-${d.id}" class="disc-edit-form" style="display:none">
      <div class="disc-edit-grid">
        <div class="fg"><label>Peso</label><input type="number" id="de-peso-${d.id}" value="${d.peso}" min="1" max="100"></div>
        <div class="fg"><label>Meta (%)</label><input type="number" id="de-meta-${d.id}" value="${d.metaAcerto||80}" min="50" max="100"></div>
        <div class="fg"><label>Cor</label><input type="color" id="de-cor-${d.id}" value="${d.cor}" class="input-color"></div>
      </div>
    ...
    <div id="disc-aulas-${d.id}" class="disc-aulas-section">
      ${sortedAulas(d.aulas).map(a=>`<div class="aula-row">
        ...
        <div class="aula-row-actions">
          ${S.disciplinas.filter(x=>x.id!==d.id).length>0?`
            ...
            <button class="btn-icon" title="Mover" onclick="moverAula('${d.id}','${a.id}')">↗</button>`:''}
...
window.closeAddDisc = ()=>{ document.getElementById('addDiscForm').style.display='none'; ['nd-nome','nd-peso','nd-meta'].forEach(id=>{ const el=document.getElementById(id); if (el) el.value=id==='nd-peso'?'20':id==='nd-meta'?'80':''; }); document.getElementById('nd-cor').value='#3b82f6'; document.getElementById('nd-nivel').value='nunca'; };
window.salvarNovaDisc = function() {
  ...
  S.disciplinas.push({id:uid(),nome,peso:parseInt(document.getElementById('nd-peso').value)||20,metaAcerto:parseInt(document.getElementById('nd-meta').value)||80,cor:document.getElementById('nd-cor').value,nivelConhecimento:document.getElementById('nd-nivel').value,aulas:[]});
...
window.removeDisc = function(id) {
  ...
  if (!confirm(`Remover "${d.nome}" e todas as suas aulas? Esta ação não pode ser desfeita.`)) return;
...
window.toggleEditDisc = function(id) {
  const v=document.getElementById(`disc-view-${id}`), e=document.getElementById(`disc-edit-${id}`); if (!v||!e) return;
  const ed=e.style.display!=='none'; v.style.display=ed?'flex':'none'; e.style.display=ed?'none':'block';
};
...
window.deleteAula = function(discId,aulaId) {
  ...
  if (!confirm(`Excluir "${a.codigo} — ${a.titulo}" e suas ${a.tarefas.length} tarefa(s)?`)) return;
...
window.moverAula = function(fromId,aulaId) {
  ...
  from.aulas=from.aulas.filter(a=>a.id!==aulaId); to.aulas.push(aula);
...
window.renderTemplates = async function() {
  ...
  container.innerHTML=`<div style="color:var(--tx3);font-size:13px;padding:20px 0">Carregando templates...</div>`;
  try {
    const snap=await db.collection('templates').orderBy('criadoEm','desc').limit(30).get();
    if (snap.empty){ container.innerHTML=`<div class="empty-state"><div class="empty-state-icon">📋</div><div class="empty-state-title">Nenhum template disponível ainda</div><div class="empty-state-sub">Seja o primeiro! Clique em "Publicar meu plano" para compartilhar seu plano com outros usuários.</div></div>`; return; }
    container.innerHTML=snap.docs.map(doc=>{ const t=doc.data(); const isOwn=t.autorUid===currentUser?.uid;
      return `<div class="template-card">
        <div class="template-header">
          <div>
            <div class="template-nome">${t.nome} ${isOwn?'<span class="own-badge">SEU</span>':''}</div>
  ...
  } catch(e){ container.innerHTML=`<div class="alert-box alert-info">Erro ao carregar: ${e.code||e.message}</div>`; }
};
...
window.publicarTemplate = async function() {
  ...
  const templateDiscs=JSON.parse(JSON.stringify(S.disciplinas)).map(d=>({...d,aulas:d.aulas.map(a=>({...a,tarefas:a.tarefas.map(t=>({...t,status:'pendente',acertoQuestoes:undefined}))}))}));
  const template={id:uid(),nome,autor:currentUser.displayName||'Anônimo',autorUid:currentUser.uid,disciplinas:templateDiscs,criadoEm:new Date().toISOString()};
  try { await db.collection('templates').doc(template.id).set(template); showToast('Template publicado!'); goTab('templates'); }
  catch(e){ showToast(`Erro: ${e.code}`,'error'); }
};

window.importarTemplate = async function(id) {
  if (!confirm('Importar este template? As disciplinas serão ADICIONADAS ao seu plano (sem apagar o que você já tem).')) return;
  try {
    const snap=await db.collection('templates').doc(id).get(); if (!snap.exists){ showToast('Template não encontrado.','error'); return; }
    const t=snap.data();
    const novas=JSON.parse(JSON.stringify(t.disciplinas)).map(d=>({...d,id:uid(),aulas:d.aulas.map(a=>({...a,id:uid(),tarefas:a.tarefas.map(x=>({...x,id:uid(),status:'pendente'}))}))  }));
    S.disciplinas.push(...novas); saveState(); renderDiscList(); populateDiscDropdowns();
    showToast(`${novas.length} disciplina(s) importadas!`); irParaDisciplinas();
  } catch(e){ showToast(`Erro: ${e.code}`,'error'); }
};

window.deletarTemplate = async function(id) {
  if (!confirm('Remover seu template da plataforma?')) return;
