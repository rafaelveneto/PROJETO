...
function renderDiscList() {
  ...
  html += '<button class="btn btn-g" style="font-size:11px" onclick="renomearConcurso()" title="Renomear concurso atual">✏️</button>';
  if (concs.length > 1) {
...
window.renomearConcurso = function() {
  const ativo = (S.concursos||[]).find(function(c){ return c.id===S.concursoAtivo; });
  if (!ativo) return;
  const novoNome = prompt('Novo nome para "' + ativo.nome + '":');
  if (!novoNome || !novoNome.trim()) return;
  ativo.nome = novoNome.trim();
  saveState(); renderDiscList();
  showToast('Concurso renomeado para "' + ativo.nome + '"');
};
  ...
  try {
    ...
    container.innerHTML = snap.docs.map(doc => {
      ...
      if (editing) {
        ...
        if (!window._editingTemplate.disciplinas.length) {
          html += '<div style="color:#71717a;font-size:12px;padding:8px 0">Nenhuma disciplina. Adicione abaixo.</div>';
        } else {
          window._editingTemplate.disciplinas.forEach(function(d, idx) {
        ...
        const nomesNoTemplate = window._editingTemplate.disciplinas.map(function(d){ return d.nome.toLowerCase(); });
...
window.cancelarEdicaoTemplate = function() {
  window._editingTemplate = null; renderTemplates();
};

window.removerDiscDoTemplate = function(idx) {
  if (!window._editingTemplate) return;
  window._editingTemplate.disciplinas.splice(idx, 1);
  renderTemplates();
};
...
window.adicionarDiscAoTemplate = function(discId) {
  ...
  if (!disc) return;
  const nomesAtuais = window._editingTemplate.disciplinas.map(function(d){ return d.nome.toLowerCase(); });
  ...
  window._editingTemplate.disciplinas.push(copia);
  renderTemplates();
};
...
window.salvarEdicaoTemplate = async function() {
  ...
  try {
    await db.collection('templates').doc(window._editingTemplate.id).update({
      disciplinas: window._editingTemplate.disciplinas
    });
    showToast('Template atualizado! (' + window._editingTemplate.disciplinas.length + ' disciplinas)');
    window._editingTemplate = null; renderTemplates();
